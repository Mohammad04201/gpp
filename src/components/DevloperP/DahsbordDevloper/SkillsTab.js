...
function SkillsTab({ currentDeveloper }) {
  ...
  if (!userData) {
    return (
      <div className={`dashboard-skills transition-all duration-300 ${
        ...
        <div className="text-center py-8">
          ...
          <p className={`mt-4 transition-all duration-300 ${
        ...
        {(userData.skills?.length || 0) === 0 ? (
          <div className="text-center py-8">
            <div className={`w-16 h-16 rounded-xl flex items-center justify-center mx-auto mb-4 transition-all duration-300 ${
              isDarkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-gray-100 border-gray-200 shadow-lg'
            }`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {userData.skills.map((skill, index) => (
              <div key={index} className={`rounded-lg p-4 border transition-all duration-300 ${
                isDarkMode 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {userData.projects.map((project, index) => (
              <div key={index} className={`rounded-lg p-4 border transition-all duration-300 ${
                ...
                <h4 className={`text-lg font-bold mb-3 transition-all duration-300 ${
                  isDarkMode ? 'text-white' : 'text-gray-800'
                }`}>{project.name}</h4>
                <div className="flex flex-wrap gap-2 mb-3">
                  {project.tech?.map((tech, techIndex) => (
                    <span key={techIndex} className={`px-2 py-1 rounded text-xs border transition-all duration-300 ${
                      isDarkMode 
                        ? 'bg-teal-600/30 text-teal-300 border-teal-500/30' 
